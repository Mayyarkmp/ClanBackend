...
SETTINGS = getattr(settings, 'ACTIVITY_SETTINGS', {})
...
def get_action_manager():
    """
    Returns the class of the action manager to use from ACTIVITY_SETTINGS['MANAGER']
    """
    mod = SETTINGS.get('MANAGER', 'activity.managers.ActionManager')
    try:
        return import_obj(mod)()
    except ImportError:
        raise ImproperlyConfigured(f'Cannot import {mod} try fixing ACTIVITY_SETTINGS[MANAGER] setting.')
