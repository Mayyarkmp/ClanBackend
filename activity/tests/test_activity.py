...
class ActivityTestCase(DataTestCase):
    ...
    def test_aauser1(self):
        self.assertSetEqual(
            ...
            [
                "admin commented on CoolGroup {self.timesince} ago",
                "admin started following Two {self.timesince} ago",
                "admin joined CoolGroup {self.timesince} ago",
            ],
        )

    def test_user2(self):
        self.assertSetEqual(
            actor_stream(self.user2),
            [
                "Two started following CoolGroup {self.timesince} ago",
                "Two joined CoolGroup {self.timesince} ago",
            ],
        )

    def test_user4(self):
        self.assertSetEqual(
            actor_stream(self.user4),
            [
    ...
    def test_following_with_flag(self):
        self.assertCountEqual(
            list(following(self.user4, flag="liking")), [self.another_group, self.user1]
        )
        self.assertEqual(
            list(following(self.user4, flag="watching")), [self.another_group]
        )
        self.assertEqual(list(following(self.user4, flag="blacklisting")), [self.user3])
        self.assertEqual(
            list(following(self.user4, self.User, flag="liking")), [self.user1]
        )
    ...
    def test_followers_with_flag(self):
        self.assertEqual(
            list(followers(self.another_group, flag="liking")), [self.user4]
        )
        self.assertEqual(
            list(followers(self.another_group, flag="watching")), [self.user4]
        )
    ...
    def test_stream_with_flag(self):
        self.assertSetEqual(
            user_stream(self.user4, follow_flag="blacklisting"),
    ...
    def test_doesnt_generate_duplicate_follow_records(self):
        ...
        follows = Follow.objects.filter(
            user=s, object_id=g.pk, content_type=self.group_ct
        )
        self.assertEqual(1, follows.count(), "Should only have 1 follow record here")
        ...
        self.assertEqual(
            1, follows.count(), "Should still only have 1 follow record here"
        )
    ...
    def test_tag_follow_url(self):
        ...
        self.assertEqual(
            output, reverse("actstream_follow", args=(self.user_ct.pk, self.user1.pk))
        )
    ...
    def test_tag_follow_all_url(self):
        ...
        self.assertEqual(
            output,
            reverse("actstream_follow_all", args=(self.user_ct.pk, self.user1.pk)),
        )
    ...
    def test_tag_actor_url(self):
        ...
        output = render(src, user=self.user1)
        self.assertEqual(
            output, reverse("actstream_actor", args=(self.user_ct.pk, self.user1.pk))
        )
    ...
    def test_tag_activity_stream(self):
        output = render(
            """{% activity_stream 'actor' user as='mystream' %}
        ...
        self.assertAllIn([str(action) for action in actor_stream(self.user1)], output)
    ...
    def test_user_stream_with_kwargs(self):
        ...
        """
        self.assertSetEqual(
            user_stream(self.user1, verb="joined"),
            [
                "Two joined CoolGroup %s ago" % self.timesince,
            ],
        )
    ...
    def test_is_following_tag_with_flag(self):
        src = (
            '{% is_following user group "liking" as is_following %}'
            "{% if is_following %}yup{% endif %}"
        )
        self.assertEqual(render(src, user=self.user4, group=self.another_group), "yup")
        self.assertEqual(render(src, user=self.user1, group=self.another_group), "")
    ...
    def test_is_following_tag_with_verb_variable(self):
        src = (
            "{% is_following user group verb as is_following %}"
            "{% if is_following %}yup{% endif %}"
        )
        self.assertEqual(
            render(src, user=self.user4, group=self.another_group, verb="liking"), "yup"
        )
        self.assertEqual(
    ...
    def test_with_user_activity(self):
        ...
        self.assertIn(
            self.join_action, list(user_stream(self.user1, with_user_activity=True))
        )
