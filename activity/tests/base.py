...
from activity.models import Action, Follow
from activity.registry import register, unregister
from activity.actions import follow
...
class ActivityBaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        register(self.User)
        for model in self.activity_models:
            register(model)
...
class DataTestCase(ActivityBaseTestCase):
    activity_models = ('auth.Group', 'sites.Site')
